...
public:
  ...
  INTERVAL (BIASINTERVAL& IN)
  { BiasHullRR(& ival, & IN.inf, & IN.sup);}
  ...
  REAL sup(){return ival.sup;}
  REAL inf(){return ival.inf;}
  REAL diam(){
      return BiasDiam(Bias(*this));
  }
  REAL mid(){return (ival.inf+ival.sup)/2.0;}
  ...
  friend INTERVAL operator / (CONST INTERVAL &, REAL            );
  friend INTERVAL operator / (CONST INTERVAL &, CONST INTERVAL &);
  friend INTERVAL operator ^ (CONST INTERVAL &, INT             );
  ...
  friend bool IsNAN(CONST INTERVAL& );
};
...
/**********************************************************************
*  R = Intersection (A, B). R is undefined, if A and B 
 * do not intersect
