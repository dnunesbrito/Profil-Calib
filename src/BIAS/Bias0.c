  ...
  */
{
  ...
  if (ZeroM (SECUREREAL (pb))) _BiasError (ErrorDivideByZero);
#endif
  _BiasRoundDown ();
  pR->inf = SECUREREAL (pa) / SECUREREAL (pb);
  _BiasRoundUp ();
  ...
  */
{
  ...
  if ((Sign(pB->sup) == 0) && (Sign(pB->inf) || ZeroM(pB->inf)))
    {
#if defined (__BIASRAISEDIVIDEBYZERO__)
      _BiasError (ErrorDivideByZero);
#else
      if (ZeroM (SECUREREAL (pa))) pR->sup = pR->inf = BiasNaN;
  ...
  */
{
  _BiasRoundDown ();
  if (Sign (SECUREBIASINTERVAL (pA)->inf) == 0) {                            /* A >= 0 */
    if ((Sign (SECUREBIASINTERVAL (pB)->inf) == 0) && !ZeroM (SECUREBIASINTERVAL (pB)->inf)) {     /* B >  0 */
      pR->inf = SECUREBIASINTERVAL (pA)->inf / SECUREBIASINTERVAL (pB)->sup;
      _BiasRoundUp ();
      pR->sup = SECUREBIASINTERVAL (pA)->sup / SECUREBIASINTERVAL (pB)->inf;
    }
    else if (Sign (SECUREBIASINTERVAL (pB)->sup) && !ZeroM (SECUREBIASINTERVAL (pB)->inf)) {       /* B <  0 */
      pR->inf = SECUREBIASINTERVAL (pA)->sup / SECUREBIASINTERVAL (pB)->sup;
      _BiasRoundUp ();
      pR->sup = SECUREBIASINTERVAL (pA)->inf / SECUREBIASINTERVAL (pB)->inf;
    }
      ...
      {
	_BiasRoundUp();
	if (ZeroM (SECUREBIASINTERVAL (pA)->inf)) pR->sup = pR->inf = BiasNaN;
	else { pR->inf = BiasNegInf; pR->sup = BiasPosInf; }
      }
#endif
  }
    ...
    if ((Sign (SECUREBIASINTERVAL (pB)->inf) == 0) && !ZeroM (SECUREBIASINTERVAL (pB)->inf)) {     /* B >  0 */
      ...
      {
	_BiasRoundUp();
	if (ZeroM (SECUREBIASINTERVAL (pA)->sup)) pR->sup = pR->inf = BiasNaN;
	else { pR->inf = BiasNegInf; pR->sup = BiasPosInf; }
      }
#endif
  }
