...
static std::function<REAL (CONST VECTOR &)> pfunc;
static VECTOR q0, q1;
static REAL qd0, qd1;
static VECTOR x;
static MATRIX v;
...
REAL BrentMinimize_Expert (VECTOR & p, REAL hmin, REAL rtol, REAL atol,
			   ...
			   std::function<REAL (CONST VECTOR &)> pf,CONST OPTMISET& optmiset)
/*
 ...
 */
{
  ...
  v = Id(dim);
  d(1) = qd0 = 0.0; q0 = q1 = x;
  for(INT loop=0;loop < optmiset.MaxIter;loop++) {
    sf = d(1); d(1) = s = 0.0;
    ...
    for (k = 2; k <= dim; k++) {
      y = x; sf = fx;
      illc |= (kt > 0);
      for(int contrepeat = 0;contrepeat < optmiset.MaxIter ;contrepeat++) {
	repeat_it = 0;
	kl = k; df = 0.0;
	if (illc) {
	  // random step
 ...
 */
{
  return BrentMinimize_Expert (p, Norm (diam), rtol, atol, 1.0, 0, 1, pf,optmiset);
}
