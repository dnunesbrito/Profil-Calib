                                ...
                                INT Level)
{
  ...
  if (++Level == optmiset.BranchLevels)
    {
      if (IsEmpty (list))
	LocalStrategy (y1.Box, ApproximationList,optmiset.MaxIter);
#ifdef SORT_LIST
    ...
    {
      RecursiveBranch (y1, ApproximationList, list, optmiset,Level);
      RecursiveBranch (y2, ApproximationList, list, optmiset,Level);
    }
}
				...
				OPTMISET& optmiset)
{
  ...
  LocalRaiseFactor = optmiset.RaiseFactor;
  LocalExpandFactor = optmiset.HeuristicExpandFactor;
  LocalTolerance = optmiset.Eps;
  CurrentUpperBound = optmiset.UpperBound;
  UsingDerivatives = optmiset.UseDerivatives;
  optmiset.BranchLevels *= Dimension (First (SolutionList).Box);
  for (i = 0; i < optmiset.Iterations; i++) {
        if (IsEmpty (SolutionList)) break;
        if(Length (SolutionList) < optmiset.MaxBranches)
            len = Length (SolutionList);
        else
            len = optmiset.MaxBranches;
        cout << "   -> Iterations: " << i+1 << " of " << optmiset.Iterations << endl;
        for (j = 0; j < len; j++) {
				...
				OPTMISET& optmiset)
{
    ...
    optmiset.BranchLevels *= Dimension (First (SolutionList).Box);
    int NewLen = Length(SolutionList);
    ...
    for (int i = 0; i < optmiset.Iterations; i++) {
        ...
        for (j = 0; j < len; j++) {
            ...
            y = First (SolutionList);
            --SolutionList;
            Branch (y,ContSaveFile, ApproximationList, NewSolutionList, optmiset);
#ifdef SORT_LIST
            if(Length(NewSolutionList) + Length(SolutionList) > optmiset.MaxBranches){
                NewLen = Length(SolutionList);
                continue;
            }
            while (!IsEmpty (NewSolutionList)) {
                SolutionList2 <<= First (NewSolutionList);
            ...
            Append (SolutionList, NewSolutionList);
            NewLen = Length(SolutionList) + ContFile*FILELEN;
            UpdateLowerBound (SolutionList, optmiset.LowerBound);
            if(!IsEmpty(SolutionList)){
                if(Length(SolutionList) > FILELEN){
                    First(SolutionList);
                    ...
                    ContFile++;
                    UpdateLowerBound (SolutionList, optmiset.LowerBound);
                    if(ContLoadFile >= 1)ContLoadFile--;
                }
            }
#endif
        }
                                     ...
                                     OPTMISET& optmiset)
{
    ...
    optmiset.UpperBound = Machine::PosInfinity;
    SortDiam (FunctionDomain, maxdir);
    list<SOLUTIONLIST_ELEMENT> SolutionList;
    Append (SolutionList, FunctionDomain, SymHull (Machine::PosInfinity));
    YAML::Node configfilenode;
