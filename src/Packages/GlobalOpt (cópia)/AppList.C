...
void SaveAprroxList(std::string FileName,APPROXIMATIONLIST& IN){
    APPROX_ELEMENT Approx_element;
    FileOp SaveList(FileName);
    Approx_element = First(IN);
    SaveList.node.reset();
    SaveList.node["APPROXLIST"].push_back(Length(IN));
    while(!Finished(IN)){
        YAML::Node elementnode;
        WriteNode(elementnode,Approx_element.ExpansionBox);
        WriteNode(elementnode,Approx_element.HeuristicBox);
        WriteNode(elementnode,Approx_element.MinEnclosure);
        WriteNode(elementnode,Approx_element.MinPoint);
        WriteNode(elementnode,Approx_element.MinValue);
        WriteNode(elementnode,Approx_element.MinValueEnclosure);
        SaveList.node["APPROXLIST"].push_back(elementnode);
        Approx_element = Next(IN);
    }
    First(IN);
}

void LoadAprroxList(std::string FileName,APPROXIMATIONLIST& IN){
    APPROXIMATIONLIST Approximationlist;
    APPROX_ELEMENT Approx_element;
    YAML::Node Listnode;
    FileOp SaveList(FileName);
    Listnode = SaveList.node["APPROXLIST"];
    if(Listnode.IsNull()){
        cerr << "The list isn't approximation list" << endl;
        return;
    }
    for(int contEl = 1;contEl < Listnode.size();contEl++){
        ReadNode(Listnode[contEl],0,Approx_element.ExpansionBox);
        ReadNode(Listnode[contEl],1,Approx_element.HeuristicBox);
        ReadNode(Listnode[contEl],2,Approx_element.MinEnclosure);
        ReadNode(Listnode[contEl],3,Approx_element.MinPoint);
        ReadNode(Listnode[contEl],4,Approx_element.MinValue);
        ReadNode(Listnode[contEl],5,Approx_element.MinValueEnclosure);
        if(contEl == 1)
            IN *= Approx_element;
        else
            IN += Approx_element;
