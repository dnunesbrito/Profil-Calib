...
BOOL APPROX_ELEMENT::operator < (const APPROX_ELEMENT& obj){
    return (MinValue < obj.MinValue);
}
...
VOID Cleanup (std::list<APPROX_ELEMENT> & list, REAL threshold)
// Delete all list entries with Inf(F(y)) > threshold.
{
    if (list.empty()) return;
    for(std::list<APPROX_ELEMENT>::iterator it = list.begin();it != list.end();)
        if((*it).MinValue > threshold)
            it = list.erase(it);
        else
            ++it;
}
...
BOOL Expansion_Exists (std::list<APPROX_ELEMENT> & list,
                       CONST INTERVAL_VECTOR & v)
{
  if (!list.empty()){
    for (std::list<APPROX_ELEMENT>::iterator it = list.begin();it != list.end();++it)
      if (HasInclusion ((*it)) && (v <= ((*it).ExpansionBox)))
          return TRUE;
  }
  return FALSE;
}
...
BOOL Approximation_In_Range (std::list<APPROX_ELEMENT> & list,
                             CONST INTERVAL_VECTOR & v)
{
  INTERVAL_VECTOR dummy (Dimension (v));
  ...
  if (!list.empty())
    for (std::list<APPROX_ELEMENT>::iterator it = list.begin();it != list.end();++it) {
      if (HasInclusion ((*it))
	  && Intersection (dummy, v, (*it).ExpansionBox))
	return TRUE;
      if (Intersection (dummy, v, (*it).HeuristicBox)) return TRUE;
    }
  return FALSE;
}