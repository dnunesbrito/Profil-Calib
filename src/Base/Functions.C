...
INTERVAL Constant::ZERO   = INTERVAL(0,0);
INTERVAL Constant::UM     = INTERVAL(1,1);
...
INTERVAL ArcTan2 (CONST INTERVAL & y,CONST INTERVAL & x){
    INTERVAL r;
    r = y/x;
    if((0.0 <= x) && (0.0 <= y)){
        return INTERVAL(0,0);
    ...
    if(Inf(x) > 0){
        return ArcTan(r);
    }else if(Sup(x) < 0 && Inf(y) >= 0){ 
        return ArcTan(r)+Constant::Pi;
    }else if(Sup(x) < 0 && Inf(y) <  0){
        return ArcTan(r)-Constant::Pi;
    }else if((0.0 <= x) && Sup(y) > 0){
        INTERVAL tmpx(Inf(r),-Machine::MinPositive);
        tmpx = ArcTan(tmpx);
        INTERVAL tmpx2(Machine::MinPositive,Sup(r));
        tmpx2 = ArcTan(tmpx2);
        return tmpx+Constant::Pi/2+tmpx2;
    }else if((0.0 <= x) == 0 && Sup(y) < 0){
        INTERVAL tmpx(Inf(r),-Machine::MinPositive);
        tmpx = ArcTan(tmpx);
        INTERVAL tmpx2(Machine::MinPositive,Sup(r));
        tmpx2 = ArcTan(tmpx2);
